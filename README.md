...
That version is a minimum, per Kodi's `<import>` semantics. CoreELEC's build
appends its own fourth component, so a module reporting `1.3.0.0` in Kodi
still satisfies an import of `1.3.0`; import the three-component version.

The module registers as an `xbmc.python.module` extension point, so `import
sidedata` works once the dependency is declared:
...
The result is a dict with seven keys: `flags`, `structure`, `config`, `rpu`,
`hdr10plus`, `mdcv`, `cll`. Each one is `None` when its payload is absent or
unparseable, except `flags`, which is `[]` instead. [FIELDS.md](FIELDS.md)
is the field-by-field reference, kept current with a changelog per release.
...
and `rpu.l6.max_cll` becomes `sidedata.rpu.l6.max_cll`:

```xml
$INFO[Window(Home).Property(sidedata.rpu.profile)]
```

A few sections hold lists rather than a single dict and need their own
naming rule. `flags`, `hdr10plus.maxscl` and `hdr10plus.bezier_anchors`
...
`target_display_index` instead, since two L10 blocks can share a nits
value, with `sidedata.rpu.l10.indexes` listing the indexes present.
HDR10+'s distribution keys by percentile the same way, so
`sidedata.hdr10plus.distribution.50` holds the 50th percentile's nits value
and `sidedata.hdr10plus.distribution.percentages` lists the percentiles
present.

Two blocks in the same section can resolve to the same key, two 300 nit L2
trims, say. The first keeps the plain key and each later one gets a dash
and an ordinal appended, so a second 300 nit L2 trim publishes at
`sidedata.rpu.l2.300-2` rather than overwriting the first. The enumeration
property for that section, `sidedata.rpu.l2.nits` and its siblings, lists
the exact tokens in the order the blocks appeared, dash suffixes included,
so a skin can walk every one of them by taking each token as the next path
segment.
